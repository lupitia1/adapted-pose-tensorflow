...
def get_batch_spec(cfg):
    ...
    batch_spec = {
        Batch.inputs: [batch_size, 200, 200, 3],
        Batch.part_score_targets: [batch_size, 26, 26, num_joints],
        Batch.part_score_weights: [batch_size, 26, 26, num_joints]
    }
    if cfg.location_refinement:
        batch_spec[Batch.locref_targets] = [batch_size, 26, 26, num_joints * 2]
        batch_spec[Batch.locref_mask] = [batch_size, 26, 26, num_joints * 2]
    if cfg.pairwise_predict:
        batch_spec[Batch.pairwise_targets] = [batch_size, 26, 26, num_joints * (num_joints - 1) * 2]
        batch_spec[Batch.pairwise_mask] = [batch_size, 26, 26, num_joints * (num_joints - 1) * 2]
    return batch_spec
